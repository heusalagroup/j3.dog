...
export async function main (
    ...
    try {
        ...
        const gridService : J3GridService = new J3GridServiceImpl();

        J3BackendControllerImpl.setGridService(gridService);

        const server = new RequestServer(BACKEND_URL);
        server.attachController(J3BackendControllerImpl);
        server.start();
